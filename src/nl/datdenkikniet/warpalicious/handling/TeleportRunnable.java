          ...
          if (warp.getLocation(false) != null && warp.getLocation(false).getWorld() != null) {
            if (mode.getEffect(Direction.DEPART) != null && !str
              ...
              player.getLocation().getWorld()
                  .spawnParticle(mode.getEffect(Direction.DEPART), player.getLocation(),
            ...
            player.sendMessage(str.warpToWarp.replace("%NAME%", warp.getName())
                .replace("%WORLDNAME%", warp.getLocation(false).getWorld().getName()));
            if (mode.getEffect(Direction.ARRIVAL) != null && !str
                .checkPermission(player, str.noParticlePerm)) {
              player.getLocation().getWorld()
                  .spawnParticle(mode.getEffect(Direction.ARRIVAL), player.getLocation(),
                      mode.getEffectCount(Direction.ARRIVAL));
            }

          } else {
            player.sendMessage(
                str.invalidWorld.replace("%WARPNAME%", warp.getName()).replace("%WORLDNAME%", ""));
          }
        }
        timeLeft--;
      } else {
