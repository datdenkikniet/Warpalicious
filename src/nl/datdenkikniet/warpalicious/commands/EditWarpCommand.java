...
public class EditWarpCommand implements CommandExecutor {
    ...
    public EditWarpCommand(Strings str, WarpHandler handler) {
        this.str = str;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (PermissionStorage.hasPermission(sender, PermissionStorage.EDIT)) {
            if (args.length == 1 && args[0].equalsIgnoreCase("listflags")) {
                ...
                if (warp != null) {
                    if (warp.getOwner().equals(player.getUniqueId()) || PermissionStorage.hasPermission(sender, PermissionStorage.UNIVERSAL)) {
                        boolean value;
                ...
                if (handler.isFlag(args[1])) {
                    ...
                    if (warp != null) {
                        if (warp.getOwner().equals(player.getUniqueId()) || PermissionStorage.hasPermission(sender, PermissionStorage.UNIVERSAL)) {
                            boolean value = MessageGenerator.parseBoolean(args[2]);
